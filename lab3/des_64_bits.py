...
def bit_array_to_str(array):
    ...
    for i in range(0, len(array), 8):
        byte = array[i:i + 8]
        res += chr(int(''.join([str(bit) for bit in byte]), 2))
    return res
...
def des_encrypt(block, keys):
    ...
    for round_key in keys:
        temp_right = right[:]
        right = xor(left, feistel(right, round_key))
        left = temp_right
    combined = right + left
...
def des(text, key, action='encrypt'):
    if len(key) < 8:
        raise ValueError("Ключ повинен бути не менше 8 символів.")
    key = key[:8]  # Використовуємо перші 8 байт ключа
    key_bits = str_to_bit_array(key)
    keys = generate_keys(key_bits)
    text_blocks = [text[i:i + 8] for i in range(0, len(text), 8)]
    result = ''
    for block in text_blocks:
        block = block.ljust(8, '\0')  # Доповнення нулями
        block_bits = str_to_bit_array(block)
        if action == 'encrypt':
            processed_block = des_encrypt(block_bits, keys)
        else:
            processed_block = des_decrypt(block_bits, keys)
        result += bit_array_to_str(processed_block)
    return result


# Приклад використання
if __name__ == '__main__':
    text = input("Введіть текст (8 символів): ")
    key = input("Введіть ключ (8 символів): ")
    encrypted = des(text, key, action='encrypt')
    print(f"Зашифрований текст: {encrypted.encode('utf-8')}")
    decrypted = des(encrypted, key, action='decrypt')
